...
def run(args: argparse.Namespace) -> int:
    ...
    for row in rows:
        ...
        key_nyt = f"event_news:nyt:{eid}"
        if key_nyt in cp.completed:
            continue
        ...
        if not d0 or not d1:
            ...
            cp.completed.add(key_nyt)
            if args.guardian:
                cp.completed.add(f"event_news:guardian:{eid}")
        ...
        print(f"▶ NYT event_id={eid} {label} | {bgn}-{end} | {query[:60]}...", flush=True)
        try:
            res = sync_nyt(
                query=query,
        ...
        if args.guardian:
            key_g = f"event_news:guardian:{eid}"
            if key_g not in cp.completed:
                print(f"▶ Guardian event_id={eid} {label}", flush=True)
                try:
                    res_g = sync_guardian(
                        query=query,
...
def main() -> int:
    ...
    p.add_argument("--guardian", action="store_true", help="每届额外跑 Guardian")
    p.add_argument("--no-guardian", action="store_false", dest="guardian")
    p.set_defaults(guardian=False)
